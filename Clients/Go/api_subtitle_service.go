...
*/
func (a *SubtitleServiceApiService) GetVideosByIdByMediasourceidAttachmentsByIndexStream(ctx context.Context, id string, mediaSourceId string, index int32) (*http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Get")
		localVarPostBody   interface{}
		localVarFileName   string
	...
	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/Videos/{Id}/{MediaSourceId}/Attachments/{Index}/Stream"
	localVarPath = strings.Replace(localVarPath, "{"+"Id"+"}", fmt.Sprintf("%v", id), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"MediaSourceId"+"}", fmt.Sprintf("%v", mediaSourceId), -1)
	localVarPath = strings.Replace(localVarPath, "{"+"Index"+"}", fmt.Sprintf("%v", index), -1)
