...
/*
PersonsServiceApiService Gets credits for a person
Requires authentication as user
 * @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 * @param id The person id
@return []UserLibraryCreditsList
*/
func (a *PersonsServiceApiService) GetPersonsByIdCredits(ctx context.Context, id string) ([]UserLibraryCreditsList, *http.Response, error) {
	var (
		localVarHttpMethod = strings.ToUpper("Get")
		localVarPostBody   interface{}
		localVarFileName   string
		localVarFileBytes  []byte
		localVarReturnValue []UserLibraryCreditsList
	)

	// create path and map variables
	localVarPath := a.client.cfg.BasePath + "/Persons/{Id}/Credits"
