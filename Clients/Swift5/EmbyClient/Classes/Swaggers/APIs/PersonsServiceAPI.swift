...
open class PersonsServiceAPI {
  ...
  "Items" : [ {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getPersonsByName(name: String, userId: String? = nil, completion: @escaping ((_ data: BaseItemDto?,_ error: Error?) -> Void)) {
        getPersonsByNameWithRequestBuilder(name: name, userId: userId).execute { (response, error) -> Void in
            completion(response?.body, error)
        }
    }
  ...
  "Altitude" : 7.933506881737151
     ...
     */
    open class func getPersonsByNameWithRequestBuilder(name: String, userId: String? = nil) -> RequestBuilder<BaseItemDto> {
