...
open class GenresServiceAPI {
  ...
  "Items" : [ {
    ...
    open class func getGenresByName(name: String, userId: String? = nil, completion: @escaping ((_ data: BaseItemDto?,_ error: Error?) -> Void)) {
        getGenresByNameWithRequestBuilder(name: name, userId: userId).execute { (response, error) -> Void in
            completion(response?.body, error)
        }
    }
  ...
  "Altitude" : 7.933506881737151
     ...
     */
    open class func getGenresByNameWithRequestBuilder(name: String, userId: String? = nil) -> RequestBuilder<BaseItemDto> {
        var path = "/Genres/{Name}"
